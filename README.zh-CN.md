...
#### 对文件和目录进行读取
> 接受 `@py3p.monitor` 的类型检查
- 参数 `path: str` 为要读取的文件或目录的路径
- 参数 `*args: str` 为读取文件的编码或计算哈希值的算法
#### 行为
> `path` 参数为目录时，将递归获取目录结构和文件内容，保存为 `dict`
- 对于目录，将目录名作为 `key`，递归读取的目录内容作为 `value`
- 对于文件，将文件名作为 `key`，以 `args` 指定方式读取文件作为 `value`
> `path` 参数为文件时，将依次尝试以 `args` 指定方式读取文件或计算哈希值
- 如果没有传入参数或传入参数为 `None`，将按字节读取文件为十六进制字符串
- 传入参数为哈希算法，将以指定算法计算文件哈希值
- 传入参数为编码方式，将以指定编码读取文件为字符串
- 如果以指定编码读取文件时，文件出现编码错误，则尝试下一个参数
- 如果所有参数尝试后未能获得有效结果，返回 `None`
### **py3p.save**
#### 保存文件或目录到磁盘
> 接受 `@py3p.monitor` 的类型检查
- 参数 `root: str` 为要保存文件或目录到的路径
- 参数 `data` 为要保存的文件或目录数据
- 参数 `hexdigest: bool` 为是否尝试解码十六进制字符串，默认为 `False`
#### 行为
- 对于 `root` 中的 `/` 和 `\`，为保证结果，采用 URL 编码转义
- 当 `data` 为 `dict` 时，将递归保存为目录，内容对应目录结构
- 当 `data` 非 `dict` 时，将转为字符串并保存为文件，`hexdigest` 决定是否尝试解码十六进制字符串
## 许可
